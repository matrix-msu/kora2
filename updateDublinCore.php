...
//  Changelog:
// v1.1.1 - fixed data format issue causing tracker 2807793
...
echo '<h2>'.gettext('Updating Dublin Core Data for').' '.$_SESSION['currentSchemeName'].' '.gettext('in').' '.htmlEscape($_SESSION['currentProjectName']).'</h2>';

if(isset($_POST['refresh'])) {
   $dcfields = getDublinCoreFields($_SESSION['currentScheme'], $_SESSION['currentProject']);
   echo gettext('updating data').'...';
   //delete the current data...
   $query = "DELETE FROM dublinCore WHERE pid=$_SESSION[currentProject] AND sid=$_SESSION[currentScheme]";
   $db->query($query);
   $controlids = array('0');
   ...
   if(!empty($dcfields)){
   		...
   		$timequery = "SELECT cid FROM p$_SESSION[currentProject]Control WHERE name='systimestamp' AND schemeid='$_SESSION[currentScheme]' LIMIT 1";
   ...
   $query = "SELECT DISTINCT id FROM p$_SESSION[currentProject]Data WHERE cid IN (";
   $query .= $controls;
   $query .= ") AND schemeid=$_SESSION[currentScheme] AND value != ''";
   ...
   if($result->num_rows != 0) {
      ...
      foreach($records as $record) {
         ...
         $query = "SELECT * from p$_SESSION[currentProject]Data WHERE cid IN ($controls) AND schemeid=$_SESSION[currentScheme] and value != ''
          ...
          $query .= ") values ('$record','$_SESSION[currentProject]','$_SESSION[currentScheme]'";
          $xmlarray = array();  
   ...
   $db->query('UPDATE scheme SET dublinCoreOutOfDate=0 WHERE schemeid='.$_SESSION['currentScheme'].' LIMIT 1');
   echo '<br /><br />'.gettext('done').'!';
}
else {
   $query = "SELECT * FROM dublinCore WHERE pid=$_SESSION[currentProject] AND sid=$_SESSION[currentScheme] LIMIT 1";
   $result = $db->query($query);
   $query2 = "SELECT dublinCoreFields FROM scheme WHERE pid=$_SESSION[currentProject] AND schemeid=$_SESSION[currentScheme] AND dublinCoreFields IS NOT NULL";
...
if($dcfields) {
   ...
   foreach($dcfields as $dctype => $ids) {
      if(count($ids) > 0) {
        ...
        $query = "SELECT cid,name FROM p$_SESSION[currentProject]Control WHERE cid IN (".implode(',',$ids).")";
        $result = $db->query($query);
        echo $db->error;
        while($array = $result->fetch_assoc()) {   
