...
require_once(basePathPlugin.'plugins/pluginsInclude.php');

include_once(basePathPlugin.'includes/gettextSupport.php');	// need to include this to access $locale_list

// HOLY COW, TAKING THESE MENUS TO ARRAY-DEFS HERE INSTEAD OF TERRIBLE TEXT-BLOBS THAT CAME OUT OTHERWISE
$menu_nologin = [
	baseUrlPlugin.'index.php' => gettext('Log In'),
	baseUrlPlugin.'accountRegister.php' => gettext('Register an Account'),
	baseUrlPlugin.'accountActivate.php' => gettext('Activate an Account')
	];

$menu_sysadmin = [
	baseUrlPlugin.'manageProjects.php' => gettext('Create/Modify Projects'),
	baseUrlPlugin.'manageSearchTokens.php' => gettext('Manage Search Tokens'),
	baseUrlPlugin.'manageUsers.php' => gettext('Manage Users'),
	baseUrlPlugin.'systemManagement.php' => gettext('System Management')
	];

$menu_proj = [
	baseUrlPlugin.'selectProject.php' => gettext('Select a Project')
	];
	
$menu_scheme = [
	baseUrlPlugin.'selectScheme.php' => gettext('Select a Scheme')
	];
	
$menu_record = [
	baseUrlPlugin.'searchResults.php' => gettext('List Scheme Records')
	];
...
$menu_pluginSettings = [
	baseUrlPlugin.'pluginSettings.php' => gettext('Plugin Settings')
];
...
{	
	if($plugin['Enabled']==1){
		...
		for($i = 0; $i < count($menuArray) ; $i++)
		{
			...
			$menu_plugin[baseUrlPlugin."plugins/".$html.$keyVal[1]] = $keyVal[0];	
		}
...
print "</div> <!-- left -->\n";
// HELPER FUNCTIONS
function PrintMenu($title_, $items_, $type_='')
{	
