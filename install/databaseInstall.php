...
if(isset($_POST["submit"])) { 
	...
	if(isset($_POST['generateConf'])){
		...
		$createConfig=$controlUpdate=false;
	}
   ...
   if (isset($_POST['newDB']))
   {
   		...
   		$newdbname = "kora";
   		$dbuser = $_POST['rootDBUser'];
   		$dbpass = $_POST['rootDBPass'];
   }
   	else{
   ...
   if(isset($_POST['koraAdminPass']) && !empty($_POST['koraAdminPass']))
       $createAdmin = true;
   if(isset($_POST['koraDBPrefix']) && !empty($_POST['koraDBPrefix']))
       $dbprefix = true; 
   if(isset($_POST['koraDBUser']) && isset($_POST['koraDBPass']) && !empty($_POST['koraDBUser']) && !empty($_POST['koraDBPass']))
       $createKoraDBUser = true;
   
   //connect to server
   $dbLink = mysql_connect($_POST['dbHost'],$dbuser,$dbpass); //the username must be pre-existing
   ...
   {
      if($dbprefix)
        $newdbname = $_POST['koraDBPrefix']."_".$newdbname;
       if(!mysql_query('DROP DATABASE IF EXISTS '.$newdbname, $dbLink))
   	die('!! Unable to drop old'.$newdbname.'database.<br/>'.mysql_error($dbLink));
       if(!mysql_query('CREATE DATABASE '.$newdbname,$dbLink))
   ...
   if($createKoraDBUser) {
    if(!mysql_query("GRANT ALL PRIVILEGES on $newdbname.* TO '$_POST[koraDBUser]' IDENTIFIED BY '$_POST[koraDBPass]' "))
           die("!! Unable to createa a database user for KORA: ".mysql_error($dbLink));
   }
   
   // select the Database
   mysql_select_db($newdbname, $dbLink);
   ...
   if($tablesresult=mysql_query($tablesquery)){
   ...
   if(isset($tables[0])){
   		foreach($tables as $table_name){
  			...
  			if(!$result = mysql_query($sql)){
    			die( "Error deleting $table_name. MySQL Error: " . mysql_error() . "<br/>");
   			}
   		}
   }
   ...
   {
       if(!mysql_query($query, $dbLink)) die("!! Unable to create table: $name<br/>");
   ...
   if ($schemeHasFile)
   {
       $query = "ALTER TABLE premisScheme0
                           ...
                           ADD COLUMN significantProperties VARCHAR(50) NULL
                               AFTER formatVersion";
       if (!mysql_query($query, $dbLink)) die('!! Unable to alter table: premis_scheme0<br/>');
       echo '++ Altered table: premis_scheme0<br/>';
   }
   
   //Create koraadmin account for this kora installation
   $kadminpass = "";
   if($createAdmin && strlen($_POST['koraAdminPass']) > 7 )
     $kadminpass = $_POST['koraAdminPass'];
   ...
   $query = "INSERT user (username,password,salt,email,realName,admin,confirmed,searchAccount) VALUES ('koraadmin','$pwhash',$salt,'$_POST[koraAdminEmail]'";
   $query .= ",'Built-in Kora Admin','1','1','0')";
   if(!mysql_query($query,$dbLink)) die('!!Unable to create koraadmin user,<br/>'.mysql_error($dbLink));
...
along with this program.  If not, see <http://www.gnu.org/licenses/>. */\n") === FALSE) die('!!! Unable to create/write to file - check your file permissions on the install directory!');
      ...
      if($createKoraDBUser)
         fwrite($filehandle,$_POST['koraDBUser']);
      else
         fwrite($filehandle,$dbuser);
      fwrite($filehandle,"\";\n".'$dbpass = "');
      if($createKoraDBUser) 
         fwrite($filehandle,$_POST['koraDBPass']);
      else
         fwrite($filehandle,$dbpass);
      fwrite($filehandle,"\";\n".'$dbname = "'.$newdbname.'";'."\n");
      fwrite($filehandle,'$dbhost = "'.$_POST['dbHost'].'";'."\n");
      ...
      fwrite($filehandle,"require_once('required.php');\n ?>");
   ...
   if($controlUpdate) {
      $dir = "../controls/";
        ...
        mysql_query('DELETE FROM control');
        // insert the controls into the table
        foreach($dbControls as $c) mysql_query("INSERT INTO control (name, file, class, xmlpacked) VALUES ('".mysql_real_escape_string($c['name'])."', '".mysql_real_escape_string($c['file'])."', '".mysql_real_escape_string($c['class'])."', '".mysql_real_escape_string($c['xmlPacked'])."')");
        ...
        $db = new mysqli($_POST['dbHost'], $dbuser, $dbpass, $newdbname); //The upgradeDatabase script expects a db object to be defined for the kora db
