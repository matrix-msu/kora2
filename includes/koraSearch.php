...
//require_once(basePath.'controls/multiListControl.php');
...
function multiSort($left, $right)
{
	...
	foreach($KORA_Search_order as $ord)
	{
		...
		if(strtolower($ord['field']) == 'kid'){
			$ridl = parseRecordID($left['kid']);
			$ridr = parseRecordID($right['kid']);
			...
			if($ord['direction'] == SORT_ASC){
				...
				if(empty($l['era']) && !empty($r['era']) && $r['era'] == 'BCE' ) return 1;
					...
					if($l['era']  == 'BCE' && $r['era'] == 'CE')return -1;
						...
						return compareDates($l,$r);
					}
				}
				return compareDates($r,$l);
			}
...
/**
 * Compare two dates to sort them SORT_ASC. Does not take era into account.
 * Meant to be a helper function for multiSort().
 */
function compareDates($r,$l){
